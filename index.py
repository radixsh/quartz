...
async def get_help(ctx):
    ...
    embed.add_field(name=f"`{client.command_prefix}list` (aka `l`, `quote`, `q`)", 
...
@client.command(aliases=['q','eliQuote','l','list','eliList'])
async def quote(ctx):
    quotes = [
...
#dog
@client.command(aliases=['dog','eliDog'])
...
#cat
@client.command(aliases=['cat','eliCat'])
...
#jokes
@client.command(aliases=['joke','eliJoke'])
...
#ascii table encode
@client.command(aliases=['eli_shift_forward','forward'])
...
#ascii table decode
@client.command(aliases=['eli_shift_back','back'])
